...
@jit_class
class RVEADE(Algorithm):
    """TensorRVEA algorithm variant with Differential Evolution crossover operators.
    ...
    def ask(self, state):
        ...
        idx1 = jax.random.randint(x1_key, shape=(self.pop_size,), minval=0, maxval=self.pop_size)
        idx2 = jax.random.randint(x2_key, shape=(self.pop_size,), minval=0, maxval=self.pop_size)
        crossovered = self.crossover(x_key, population, population[idx1], population[idx2])
